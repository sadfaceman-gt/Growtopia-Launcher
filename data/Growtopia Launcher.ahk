...
LauncherVersion := Format("{:0.1f}", Version)
Global GTVersion := Format("{:0.2f}", 0.00)
Global SettingsCount := 3
...
If !A_IsAdmin {
	If FileExist(A_WorkingDir . "\.launcher\autostart"){
		MsgBox "Failed to launch Growtopia Launcher as administrator. Please manually launch Growtopia Launcher as administrator", "Growtopia Launcher", "OK T5"
		ExitApp
	}
...
GuiMain(){
	...
	EmbedSite_DQ := Main.Add("Text", "x501 y30 w393 h572 Background000000")
	...
	Main.Add("Picture", "x924 y0 BackgroundTrans",  A_WorkingDir . "\.launcher\Images\sidebar.png")
	Main.Add("Picture", "x942 y18 BackgroundTrans", A_WorkingDir . "\.launcher\Images\icon_forums.png").OnEvent("Click", FrLink)
	Main.Add("Picture", "x942 y100 BackgroundTrans", A_WorkingDir . "\.launcher\Images\icon_twitter.png").OnEvent("Click", TwLink)
	Main.Add("Picture", "x942 y182 BackgroundTrans", A_WorkingDir . "\.launcher\Images\icon_instagram.png").OnEvent("Click", IgLink)
	Main.Add("Picture", "x942 y264 BackgroundTrans", A_WorkingDir . "\.launcher\Images\icon_discord.png").OnEvent("Click", DcLink)
	Main.Add("Picture", "x942 y346 BackgroundTrans", A_WorkingDir . "\.launcher\Images\icon_github.png").OnEvent("Click", GhLink)
	; SetButton := Main.Add("Picture", "x942 y686 BackgroundTrans", A_WorkingDir . "\.launcher\Images\icon_settings.png")
	SetGameButton()
	GetSelFile()
	...
	Global EmbedView_DQ := WebView2.create(EmbedSite_DQ.Hwnd)
	EmbedView_TL.CoreWebView2.Navigate(EmbedURL[SettingsList[3]])
	EmbedView_DQ.CoreWebView2.Navigate("https://www.instagram.com/jenulotdq?theme=dark")
	GameButton.OnEvent("Click", GameButtonClick)
	; SetButton.OnEvent("Click", GuiSet)
...
MainClose(*){
	Global
	RunWait A_ComSpec . " /c taskkill /pid " . WinGetPid("ahk_exe msedgewebview2.exe"), , "Min"
	ExitApp 0
}
GameButtonClick(*){
	Global
...
IgLink(*){
	Run "https://www.instagram.com/growtopia/"
	Return
}
DcLink(*){
...
GuiSet(*){
	...
	GSet1 := Set.Add("Checkbox", "x10 y10 w300 Center", "")
	Set.Show("Center")
	Return
}
...
InstallGT(){
	...
	Install.Show("Center")
	If FileExist(DLPath . "\Growtopia-Inslatter")
		DirDelete DLPath . "\Growtopia-Installer", 1
	RunWait A_ComSpec . ' /c 7z x -aos -o"' . DLPath . '\Growtopia-Installer\" "' . DLPath . '\Growtopia-Installer.exe"', A_WorkingDir . "\.launcher\Bin\", "Min"
	...
	Loop Files DLPath . "\*.*" {
		...
		Catch
			MsgBox "Failed to delete " . A_LoopFilePath, "Growtopia Launcher", "OK T5"
	}
...
GenSettingsFile(){
	...
	FileAppend "Check launcher version`n" . SettingsList[1] . "`n", A_WorkingDir . "\.launcher\Settings"
	FileAppend "Hide launcher`n" . SettingsList[2] . "`n", A_WorkingDir . "\.launcher\Settings"
	FileAppend "Timeline destination`n" . SettingsList[1] . "`n", A_WorkingDir . "\.launcher\Settings"
}
...
GetGTVer(){
	...
	RunWait A_ComSpec " /c versionscrape.exe", A_WorkingDir . "\.launcher\Bin", "Min"
	If !FileExist(A_WorkingDir . "\.launcher\Bin\ver"){
	...
	RunWait A_ComSpec " /c sizequery.exe", A_WorkingDir . "\.launcher\Bin", "Min"
	If !FileExist(A_WorkingDir . "\.launcher\Bin\s"){
		MsgBox "Failed to launch. Check you internet connection and try again", "Growtopia Launcher", "OK T5"
		ExitApp
	}
...
DownloadFile(URL, FTarget){
	...
	RunWait A_ComSpec . ' /c ahk2exe /in "' . FDir . '\Downloader.ahk" /out "' . FDir . '\Downloader.exe" /base "AutoHotkeyUX.exe"', A_WorkingDir . "\.launcher\Bin\", "Min"
	FileDelete FDir . "\Downloader.ahk"
	If !FileExist(FDir . "\Downloader.exe"){
		DProg.Destroy()
	...
	If !WinWait("ahk_pid " . DLPID, , 10){
		RunWait A_ComSpec . " /c taskkill /pid " . DLPID, , "Min"
		DProg.Destroy()
		SetTimer DLProgQuery, 0
		FileDelete FDir . "\Downloader.exe"
		Return 1
...
^+x:: {
	...
	If !DLProgress
		Return
	Critical "On"
	If(MsgBox("Are you sure you want to cancel download?", "Growtopia Launcher", "YesNo Default2 T5") = "Yes"){
		DLInterrupt := True
		RunWait A_ComSpec . " /c taskkill /pid " . DLPID, , "Min"
		Return
	}
